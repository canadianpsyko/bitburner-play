...
export default class BasePlayer {
  ...
  get company() {
    return {
      companyName: this.data.jobs.companyName,
      multipliers: {
        rep: this.data.company_rep
      }
    }
  }
  get bladeburner() {
  ...
  get playtime() {
    return {
      ...
      sinceAug: Date.now() - this.ns.getResetInfo().lastAugReset,
      sinceBitnode: Date.now() - this.ns.getResetInfo().lastNodeReset
    }
  }
  ...
  async updateCache(repeat = true, kv = new Map()) {
    do {
      ...
      this.ns.print(`Getters: ${getters.size}`)
      getters.forEach(g => {
        old[g] = this[g];
        this.ns.print(`g: ${this[g]}`)
      })
